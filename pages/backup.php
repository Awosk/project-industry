...
// Backup klasörü — pages/ ile aynı seviyede, htdocs içinde kesin yazılabilir
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'backup') {
    ...
    foreach ($secili_tablolar as $tablo) {
        // Güvenlik: sadece mevcut tablolardan seç
...
// ═══════════════════════════════════════════════
$restore_file  = null;
$restore_tablolar = []; // yedekteki tablolar
if (isset($_GET['geri_yukle'])) {
    $dosya = basename($_GET['geri_yukle']);
    $yol   = $backup_dir . DIRECTORY_SEPARATOR . $dosya;
    if (file_exists($yol) && str_ends_with($dosya, '.sql')) {
        $restore_file = $dosya;
        // SQL dosyasını okuyup hangi tablolar var diye bak
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'restore') {
    ...
    // ── SQL'i tablo bazlı bloklara ayır ──
    // Her "DROP TABLE IF EXISTS `tablo`" satırından bir sonraki "DROP TABLE"'a kadar olan bloğu al
    $bloklar = [];
    $satirlar = explode("\n", str_replace("\r\n", "\n", $icerik));
    $aktif_tablo = null;
    $aktif_blok  = [];
    ...
    foreach ($satirlar as $satir) {
        ...
        if (preg_match('/^DROP TABLE IF EXISTS `([^`]+)`/i', $satir, $m)) {
            ...
            // Genel satırları atla (SET statements vb. dosya sonu)
    ...
    // Son bloğu kaydet
    if ($aktif_tablo !== null && !empty($aktif_blok)) {
        $bloklar[$aktif_tablo] = implode("\n", $aktif_blok);
    }

    // ── Her bloktaki SQL ifadelerini çalıştır ──
    // Büyük INSERT sorguları küçük batch'lere bölünerek gönderilir.
    // Bu sayede MySQL bağlantısı kopmaz.
    ...
    foreach ($secili_tablolar as $tablo) {
        if (!isset($bloklar[$tablo])) {
            $hatalar[] = $tablo . ': Yedekte bulunamadı';
            continue;
        }

        $blok = trim($bloklar[$tablo]);

        // Noktalı virgüle göre ifadelere böl (tırnak içi korumalı)
        $ifadeler = [];
        $tmp = '';
        $tirnak = false;
        $tirnak_char = '';
        $blok_len = strlen($blok);
        for ($i = 0; $i < $blok_len; $i++) {
            ...
            if (!$tirnak && ($c === "'" || $c === '"' || $c === '`')) {
                ...
                $tirnak_char = $c;
                $tmp .= $c;
            } elseif ($tirnak && $c === $tirnak_char && ($i === 0 || $blok[$i-1] !== '\\')) {
                $tirnak = false;
                $tmp .= $c;
            } elseif (!$tirnak && $c === ';') {
                $s = trim($tmp);
        ...
        foreach ($ifadeler as $ifade) {
            ...
            // INSERT ise satırları 200'er gruba böl
            if (stripos($ifade, 'INSERT INTO') === 0) {
                // Prefix: "INSERT INTO `tablo` (cols) VALUES" kısmı
                $values_pos = stripos($ifade, ' VALUES');
                if ($values_pos === false) {
                    // VALUES bulunamazsa direkt gönder
                    try { $pdo->exec($ifade); } catch (PDOException $e) {
                        $hatalar[] = $tablo . ': ' . $e->getMessage();
                        $tablo_hata = true;
                        break;
                    }
                    continue;
                }
                $prefix = substr($ifade, 0, $values_pos + 7); // +7 = " VALUES"
                $values_raw = trim(substr($ifade, $values_pos + 7));
                $values_raw = rtrim($values_raw, ';');

                // Satırları tırnak korumalı şekilde ayır
                $rows = [];
                $buf = '';
                $depth = 0;
                $in_str = false;
                $str_char = '';
                $vlen = strlen($values_raw);
                for ($vi = 0; $vi < $vlen; $vi++) {
                    $vc = $values_raw[$vi];
                    if (!$in_str && ($vc === "'" || $vc === '"')) {
                        $in_str = true;
                        $str_char = $vc;
                        $buf .= $vc;
                    } elseif ($in_str && $vc === $str_char && ($vi === 0 || $values_raw[$vi-1] !== '\\')) {
                        $in_str = false;
                        $buf .= $vc;
                    } elseif (!$in_str && $vc === '(') {
                        $depth++;
                        ...
                        if ($depth === 0) {
                            $rows[] = trim($buf);
                            $buf = '';
                            // Sonraki virgül ve boşlukları atla
                            while ($vi + 1 < $vlen) {
                                $next = $values_raw[$vi + 1];
                                if ($next === ',' || $next === "\n" || $next === "\r" || $next === ' ') {
                                    $vi++;
                                } else {
                                    break;
                ...
                if (empty($rows)) {
                    // Satır ayrıştırılamadıysa direkt gönder
                    try { $pdo->exec($ifade); } catch (PDOException $e) {
                        $hatalar[] = $tablo . ': ' . $e->getMessage();
                        $tablo_hata = true;
                        break;
    ...
    logYaz($pdo, 'guncelle', 'sistem',
        'Veritabanı geri yüklendi: ' . $dosya . ' — ' . implode(', ', $basarili),
        null, null, ['tablolar' => implode(',', $basarili)], 'lite');
...
<div class="card" style="border:2px solid var(--warning);">
    ...
    <div class="warn-box">
        ⚠️ <strong>Dikkat:</strong> Seçilen tablolar <strong>tamamen silinip</strong> yedekteki verilerle değiştirilecek. Bu işlem geri alınamaz.
    </div>
    ...
    <form method="post">
        ...
        <div style="margin-bottom:14px;">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:8px;">
                ...
                <label style="display:flex;align-items:center;gap:6px;padding:7px 12px;border:2px solid var(--primary);border-radius:8px;cursor:pointer;font-size:13px;font-weight:600;color:var(--primary);background:#e8f0fe;transition:.15s;"
                       id="lbl_<?= htmlspecialchars($t) ?>">
                    <input type="checkbox" name="tablolar[]" value="<?= htmlspecialchars($t) ?>" checked
                           onchange="labelGuncelle(this)"
        ...
        <div style="display:flex;gap:8px;margin-top:16px;">
            <button type="submit" class="btn btn-danger"
                    onclick="return confirm('Seçilen tablolar geri yüklenecek. Mevcut veriler silinecek!\n\nEmin misiniz?')">
                ↩️ Geri Yükle
            </button>
...
function tumunuSec(sec) {
    document.querySelectorAll('input[name="tablolar[]"]').forEach(cb => {
        cb.checked = sec;
        labelGuncelle(cb);
    });
}
    ...
    <form method="post">
        ...
        <div style="margin-bottom:12px;">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:8px;">
                ...
                <label style="display:flex;align-items:center;gap:6px;padding:7px 12px;border:2px solid var(--border);border-radius:8px;cursor:pointer;font-size:13px;font-weight:600;color:var(--text);background:#fff;transition:.15s;"
                       id="blbl_<?= htmlspecialchars($t) ?>">
                    <input type="checkbox" name="tablolar[]" value="<?= htmlspecialchars($t) ?>" checked
                           onchange="bLabelGuncelle(this)"
                           style="width:16px;height:16px;accent-color:var(--primary);">
                    <span>📋 <?= htmlspecialchars($t) ?></span>
                </label>
                <?php endforeach; ?>
        ...
        <div style="margin-top:14px;display:flex;align-items:center;gap:12px;">
            ...
            <span style="font-size:12px;color:var(--muted);">
                Yedek <code><?= htmlspecialchars(basename($backup_dir)) ?>/</code> klasörüne kaydedilir.
            </span>
        </div>
    </form>
</div>
...
<div class="card">
    <div class="card-title">
        📂 Mevcut Yedekler
        <span style="font-weight:400;font-size:13px;color:var(--muted);">(<?= count($yedekler) ?>)</span>
    </div>
    ...
    <div class="table-wrap">
        <table>
            ...
            <tr>
                <td>
                    <code style="font-size:12px;"><?= htmlspecialchars($y['dosya']) ?></code>
                </td>
                ...
                <td style="display:flex;gap:6px;flex-wrap:wrap;">
                    ...
                    <a href="?sil=<?= urlencode($y['dosya']) ?>" class="btn btn-sm btn-danger"
                       onclick="return confirm('Bu yedeği silmek istediğinizden emin misiniz?')">🗑️</a>
                </td>
            </tr>
            <?php endforeach; ?>
            </tbody>
