...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['smtp_kaydet'])) {
    ...
    $stmt = $pdo->prepare("INSERT INTO system_settings (anahtar, deger) VALUES (?, ?) ON DUPLICATE KEY UPDATE deger = VALUES(deger)");
...
$adminler = $pdo->query("SELECT id, ad_soyad, kullanici_adi, email, mail_bildirim_aktif FROM users WHERE rol = 'admin' AND aktif = 1 ORDER BY ad_soyad")->fetchAll();
