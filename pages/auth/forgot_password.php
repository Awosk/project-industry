...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$email || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $kullanici = $pdo->prepare("SELECT * FROM users WHERE email = ? AND aktif = 1");
        ...
        if ($kullanici) {
            ...
            $pdo->prepare("UPDATE users_reset_password SET kullanildi = 1 WHERE kullanici_id = ? AND kullanildi = 0")
                ->execute([$kullanici['id']]);

            $token       = bin2hex(random_bytes(32));
            $son_kullanma = date('Y-m-d H:i:s', strtotime('+30 minutes'));

            $pdo->prepare("INSERT INTO users_reset_password (kullanici_id, token, son_kullanma) VALUES (?, ?, ?)")
