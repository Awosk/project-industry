    ...
    if (empty($hatalar)) {
        try {
            ...
            if (!file_exists($sql_dosya)) {
                ...
                if (!$skip_admin) {
                    $pdo->prepare(
                        "INSERT INTO users (ad_soyad, kullanici_adi, sifre, rol) VALUES (?, ?, ?, 'admin')"
...
if ($adim === 3) {
    try {
        ...
        $tbl = $pdo_test->query("SHOW TABLES LIKE 'users'")->fetch();
        if ($tbl) {
            $say = $pdo_test->query("SELECT COUNT(*) FROM users WHERE rol = 'admin'")->fetchColumn();
...
<head>
    ...
    <style>
        ...
        .step.aktif .step-label {
            color: var(--primary-l);
        }

        .step.tamam .step-label {
            color: var(--success-text);
        }
        ...
        .feature-toggle:has(.toggle-input:checked) {
            border-color: var(--primary);
            background: rgba(30, 77, 107, .06);
        }
    ...
    <div class="login-page">
        <div class="login-box">
                ...
                <form method="post">
                    ...
                    <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:20px;">
                        <label class="feature-toggle">
                            <input type="checkbox" name="f_dashboard" value="1" class="toggle-input">
                        ...
                        <label class="feature-toggle">
                            <input type="checkbox" name="f_stok" value="1" class="toggle-input">
                            <div class="toggle-track">
                                <div class="toggle-thumb"></div>
                            </div>
                            <div class="toggle-content">
            ...
            <?php else: ?>
                ...
                <?php $kontroller = gereksinimler();
                $tumTamam = tumGereksinimleriKarsiladi(); ?>
